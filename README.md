...
- **Native MCP 自主编排中间件**:
  ...
  - **`Independent Workers` (Local-First)**:
    ...
    - **`ReportTool`**: 本地专业 PDF 报告生成 (pdf-lib)，自动探测 Unicode 字体以支持中文 PDF。
...
- 全仓 **875** 个 `Vitest` 用例（后端 **637** + 前端 **238**），覆盖 DAG 条件引擎、代理层、沙箱/MCP、`server` 关键 REST 路由、环境校验、请求关联，以及 Header / 用户壳时间线的 i18n 与可访问性回归。
