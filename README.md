...
- **企业级 SaaS 多租户隔离**:
  - **基于会话的硬隔离**：底层所有 SQLite 事务、会话上下文与长期记忆图谱表（MemoryStore）被 `user_id` 全面分区，杜绝了并发流数据交叉污染与会话错位。每个 Session 在独立的 `workspace/sessions/{sessionId}/` 目录中执行，文件互不干扰。
...
- **Native MCP 自主编排中间件**:
  ...
  - 挂载 **`BatchTool` & `LoopTool`**: 原生支持并行并发及单任务强制循环批处理流转。
...
- 高达 **509** 个的 `Vitest` 单元/集成测试用例覆盖所有的 DAG 条件引擎运算和代理层转译算法。
