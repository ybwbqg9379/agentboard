...
- **用户壳 Composer 页脚**：根层 **`WorkspaceFilesProvider`**（`sessionId` + **`events.length`** debounce）统一拉取 **`workspace-files`**，供底部 **`SessionDownloadablesStrip`** 与技术详情内 **`FileChangesPanel`** 共用；同一会话刷新时保留当前列表，切换到新会话时立即清空旧列表并进入 loading，避免旧会话下载条短暂出现在新 `sessionId` 下。`App.jsx` 将 Strip 与 **`ChatInput variant="user"`** 包在 **`user-shell-composer-footer`** 内；`index.css` 在 **`:has(.session-downloadables-dock)`** 时为页脚施加与输入区一致的 **`bg-tertiary`** 与上阴影，并去掉紧随其后的 **`form`** 顶边，使「可下载文件」与输入框视觉连成一块。进展区仅展示助手向时间线（工具细节进抽屉）。
