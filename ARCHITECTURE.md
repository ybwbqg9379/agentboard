...
| **Agent 调度** | `agentManager.js` / `agentDefs.js`     | Agent SDK 实例维护，注册预设子代理 (`code-reviewer`, `researcher` 等)，限制执行轮次与隔离环境变量。 |
| **安全沙箱**   | `hooks.js` / `mcpConfig.js`            | `PreToolUse` Bash双层围栏（拦截含绝对路径跨域文件读取如`/etc/`），MCP工具白名单阻断。               |
