...
前端在 **Agent** 模式下可选 **Console**（双栏时间线 + 侧栏）或 **Agent**（单列 `UserAgentTimeline` + 底部 `ChatInput`，侧栏进抽屉）两种壳层；**WebSocket 协议与事件模型不变**。
...
| **模块**                      | **文件**                                                                                                    | **职责**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **API与鉴权**                 | `server.js` / `http/createApp.js` / `http/routes/*` / `websocket/registerAgentBoardWs.js` / `middleware.js` | 进程入口 `server.js`：`createApp()` 挂载 REST（按域拆分至 `http/routes`），`registerAgentBoardWebSocket` 注册 `/ws` 与事件桥；`middleware` 负责 CORS、鉴权、Zod 校验。新增 `/api/sessions/:id/files/:fileName` 安全下载接口。                                                                                                                                                                                                                                                                                                  |
...
| **微服务器组**                | `nativeMcpServer.js`                                                                                        | 基于官方 `@modelcontextprotocol/sdk` 实现的后端驻留子进程，向模型动态注册高级中间件原生工具：<br/>- **`VisualizerTool`**: 本地 Mermaid 渲染引擎。<br/>- **`DataAnalystTool`**: 基于 AlaSQL 的本地极速 SQL 数据分析 (CSV/JSON)。<br/>- **`OCRTool`**: 基于 Tesseract.js 的本地图像文字识别。<br/>- **`ReportTool`**: 基于 pdf-lib 的专业 PDF 报告生成引擎，优先自动嵌入系统 Unicode 字体；缺失时回退 StandardFonts 并显式告警。<br/>- **`TaskCreateTool`**: 任务并行分发。<br/>- **`BatchTool` / `LoopTool`**: 并发与循环调度。 |
...
| **用户壳展板**   | `UserAgentTimeline.jsx`                             | 与 **Console** 壳共用同一 `events` 流；扁平化逻辑在 **`agentTimelineModel.js`**（**`buildDisplayItems`** / **`flattenEvent`** / handlers），**`AgentTimeline.jsx`** 再导出以兼容单测。行对象带稳定 **`kind`** 语义，JSON 表格 / PDF 下载增强据此判定，不依赖翻译文案；`buildDisplayItems` 会随 **`i18n.language`** 重新投影，运行时切语言可立即刷新既有里程碑。卡片式里程碑 + 动效；空态与运行中提示走 `userShell.*` i18n。                                                   |
