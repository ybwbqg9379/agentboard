...
| **微服务器组**                | `nativeMcpServer.js`                                                                                        | 基于官方 `@modelcontextprotocol/sdk` 实现的后端驻留子进程，向模型动态注册高级中间件原生工具：<br/>- **`VisualizerTool`**: 本地 Mermaid 渲染引擎。<br/>- **`DataAnalystTool`**: 基于 AlaSQL 的本地极速 SQL 数据分析 (CSV/JSON)。<br/>- **`OCRTool`**: 基于 Tesseract.js 的本地图像文字识别。<br/>- **`ReportTool`**: 基于 pdf-lib 的专业 PDF 报告生成引擎。<br/>- **`TaskCreateTool`**: 任务并行分发。<br/>- **`BatchTool` / `LoopTool`**: 并发与循环调度。 |
...
| **通用组件**  | `MarkdownBody.jsx`                                  | Markdown 渲染核心。**0.2.0 增强**：集成 `mermaid.js` 10.9.5，支持 `language-mermaid` 代码块实时拓扑图/流程图渲染。                                                                                                      |
