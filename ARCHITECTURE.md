...
| **模块**                      | **文件**                                                                                                    | **职责**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API与鉴权**                 | `server.js` / `http/createApp.js` / `http/routes/*` / `websocket/registerAgentBoardWs.js` / `middleware.js` | 进程入口 `server.js`：`createApp()` 挂载 REST（按域拆分至 `http/routes`），`registerAgentBoardWebSocket` 注册 `/ws` 与事件桥；`middleware` 负责 CORS、鉴权、Zod 校验。会话产物：**`/api/sessions/:id/files/:fileName`** 安全下载（扩展名白名单 **`shared/sessionDownloadExtensions.js`** 与前端 **`sessionDownloads.js`** 共用）；**`/api/sessions/:id/workspace-files`** 列出工作区根文件（**`stat` 后按 `mtime` 降序再取至多 500 条**；**`ENOENT`** 视为空列表，其它磁盘错误 **500**；**`CLAUDE.md`** 会话引导副本不入列表）。                                                                                                                                            |
...
| **工作区文件拉取** | `context/WorkspaceFilesProvider.jsx`                | **`App`** 根层按 **`sessionId` + `events.length`（debounce）** 单次请求 **`workspace-files`**，供 **`SessionDownloadablesStrip`** 与 **`FileChangesPanel`** 共用；失败时暴露错误态供 UI 提示。                                                                                                                                                                                                                                                                                |
...
| **数据解剖**       | `RightPanel.jsx`                                    | 3 个子 Tab：`Terminal`、`Context`、**`Files`**（**`FileChangesPanel`**：工具 **Read/Write/Edit** 与原生 **ReportTool** 统计 + **`WorkspaceFilesProvider`** 工作区列表合并；**`lib/pathBasename.js`** 对齐工具路径与根目录文件名；可下载类型显示下载入口；需 **`sessionId`**）。                                                                                                                                                                                               |
