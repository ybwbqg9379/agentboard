...
|                                                 |       |                                             |
| [Header] Shell (Console/Agent) + Mode (A/W/E)   | REST  | +-----------------+    +------------------+ |
...
前端在 **Agent** 模式下可选 **Console**（双栏时间线 + 侧栏）或 **Agent**（单列 **`UserAgentTimeline`** + 底部 **`user-shell-composer-footer`**（可选 **`SessionDownloadablesStrip`** + **`ChatInput`**），侧栏进抽屉）两种壳层；**WebSocket 协议与事件模型不变**。用户壳时间线经 **`buildUserShellDisplayItems`** 隐藏逐步工具 I/O，完整终端仍在抽屉 **`Terminal`**。

1. 用户在浏览器输入任务 (`Browser → WebSocket → { action: "start", prompt: "..." }`)；Header 的 **Console / Agent** 壳切换按钮通过 **`aria-pressed`** 暴露当前激活态，键盘与读屏路径与视觉状态一致。
...
| **模块**                      | **文件**                                                                                                    | **职责**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API与鉴权**                 | `server.js` / `http/createApp.js` / `http/routes/*` / `websocket/registerAgentBoardWs.js` / `middleware.js` | 进程入口 `server.js`：`createApp()` 挂载 REST（按域拆分至 `http/routes`），`registerAgentBoardWebSocket` 注册 `/ws` 与事件桥；`middleware` 负责 CORS、鉴权、Zod 校验。会话产物：**`/api/sessions/:id/files?path=...`** 安全下载（支持根目录或嵌套产物；旧 **`/files/:fileName`** basename 路由保留兼容；扩展名白名单 **`shared/sessionDownloadExtensions.js`** 与前端 **`sessionDownloads.js`** 共用）；**`/api/sessions/:id/workspace-files`** 列出工作区根文件（**`stat` 后按 `mtime` 降序再取至多 500 条**；**`ENOENT`** 视为空列表，其它磁盘错误 **500**；**`CLAUDE.md`** 会话引导副本不入列表）。                                                                      |
...
| **实验与评测 (AutoResearch)** | `experimentEngine.js` / `metricExtractor.js`                                                                | 新增的核心实验引擎。在 host 环境内基于白名单创建临时隔离 `workspace`，内部执行带状态恢复机制 (Git Ratchet) 的 "提议-度量-回滚/提交" 循环打分流程。baseline / guard / benchmark 采用异步可中断执行，避免长命令阻塞服务主线程；对会派生 worker 的命令，abort/timeout 会按进程组清理整个命令树。白名单扫描的 `git diff` / `git ls-files` 在屏蔽 OS 噪声 stderr 的同时仍检查退出码，扫描失败按 **fail-closed** 终止整轮实验，避免把“扫描失败”误当成“无改动”。支持直接正则/解析提取命令行输出指标，自动向 WS 事件总线广播演进度。                                                                                                                                                |
...
| **微服务器组**                | `nativeMcpServer.js`                                                                                        | 基于官方 `@modelcontextprotocol/sdk` 实现的后端驻留子进程，向模型动态注册高级中间件原生工具：<br/>- **`VisualizerTool`**: 本地 Mermaid 渲染引擎。<br/>- **`DataAnalystTool`**: 基于 AlaSQL 的本地极速 SQL 数据分析 (CSV/JSON)。<br/>- **`OCRTool`**: 基于 Tesseract.js 的本地图像文字识别。<br/>- **`ReportTool`**: 基于 pdf-lib 的 PDF 报告；**`AGENTBOARD_PDF_FONT`**（若有效）→ **捆绑 Noto Sans SC（WOFF2，npm `@fontsource/noto-sans-sc`）** → 常见 OS Unicode 路径；否则 StandardFonts 并告警。系统提示 **`[PDF REPORTS]`** 要求面向用户的交付 PDF **必须**走本工具。<br/>- **`TaskCreateTool`**: 任务并行分发。<br/>- **`BatchTool` / `LoopTool`**: 并发与循环调度。 |
...
| 模块               | 文件                                                                                | 职责                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **状态总线**       | `useWebSocket.js` / `WorkflowEditor.jsx` / `sharedWebSocket.js` / `wsConnection.js` | 管理 WS 长连接与重连；**`sharedWebSocket.js`** 以引用计数 + 短延迟释放复用浏览器侧 **`/ws`** 连接，主时间线与 Workflow 订阅共享同一租约，跨越 React **`StrictMode`** 开发期即时 remount，避免 Vite 代理侧因预期内 close 刷出 **`ECONNRESET`** 噪声；心跳 / 空闲超时 / `ping` 逻辑集中在 `wsConnection.js`。                                                                                                                                                                   |
| **主展板**         | `AgentTimeline.jsx`                                                                 | 将高嵌套维度的 SDK block 扁平化渲染为思考链（思维气泡）、执行链（终端）、报错链。**0.2.0 增强**：支持 `JsonTable`（表格化数据）、`DownloadButton`（PDF 下载）、以及对 `ReportTool` / `DataAnalystTool` 的渲染逻辑匹配。                                                                                                                                                                                                                                                       |
| **用户壳展板**     | `UserAgentTimeline.jsx`                                                             | 与 **Console** 壳共用同一 `events` 流；**`buildUserShellDisplayItems`** 过滤工具链路与 thinking，仅展示助手向内容 + 系统/统计等；**`buildDisplayItems`** 仍供 **Console** **`AgentTimeline`** 全量日志。逻辑在 **`agentTimelineModel.js`**；**`AgentTimeline.jsx`** 再导出以兼容单测。行对象带稳定 **`kind`**；`i18n.language` 变更时重投影。卡片式里程碑 + 动效；`userShell.*` i18n。                                                                                        |
| **工作区文件拉取** | `context/WorkspaceFilesProvider.jsx`                                                | **`App`** 根层按 **`sessionId` + `events.length`（debounce）** 单次请求 **`workspace-files`**，供 **`SessionDownloadablesStrip`** 与 **`FileChangesPanel`** 共用；同一会话刷新时保留当前列表，切换到新会话时立即清空旧列表并进入 loading，避免旧会话文件在新 `sessionId` 下短暂复用；失败时暴露错误态供 UI 提示。                                                                                                                                                             |
| **会话下载条**     | `SessionDownloadablesStrip.jsx`                                                     | 智能同事壳：消费 **`WorkspaceFilesProvider`**，按可下载扩展名过滤（与下载 API 白名单一致），渲染于 **`user-shell-composer-footer`** 内、**`ChatInput`** 正上方，与输入区视觉连成一体（见 **`index.css`**）。                                                                                                                                                                                                                                                                  |
| **控制台**         | `ChatInput / Header`                                                                | **Agent** 模式下 **`Console` / `Agent` 界面壳**（`data-ui-shell`，`localStorage` `agentboard-ui-shell`）。**用户壳 + Agent** 不展示模式 Tab；**控制台壳**或 **Workflow/Experiment** 下展示 **`Agent` / `Workflow` / `Experiment`** 切换。主题、调色板包（含 **`claude`**）、密度、语言。Shell 按钮用 **`aria-pressed`** 暴露当前壳层。Cmd/Ctrl+Enter 提交。`ChatInput` **`variant="user"`** 用于用户壳；主界面 **固定** `permissionMode: bypassPermissions`，无权限模式下拉。 |
| **技术详情抽屉**   | `UserAgentDetailsDrawer.jsx`                                                        | 用户壳下将 **`RightPanel`** 以侧栏/底栏抽屉呈现（终端 / 上下文 / 文件），与 Console 壳数据一致。                                                                                                                                                                                                                                                                                                                                                                              |
| **数据解剖**       | `RightPanel.jsx`                                                                    | 3 个子 Tab：`Terminal`、`Context`、**`Files`**（**`FileChangesPanel`**：工具 **Read/Write/Edit** 与原生 **ReportTool** 统计 + **`WorkspaceFilesProvider`** 工作区列表合并；**`lib/pathBasename.js`** 对齐工具路径与根目录文件名；下载链接对工具行保留完整路径、仅下载文件名显示 basename，因此嵌套产物可正确落到 **`/api/sessions/:id/files?path=...`**；需 **`sessionId`**）。                                                                                               |
...
| **通用组件**       | `MarkdownBody.jsx`                                                                  | Markdown 渲染核心。**0.2.0 增强**：集成 `mermaid.js` 10.9.5，支持 `language-mermaid` 代码块实时拓扑图/流程图渲染；渲染前使用 `textContent` 注入源码，避免 Mermaid 代码块 HTML 注入。                                                                                                                                                                                                                                                                                          |
| **通用组件**       | `ConfirmDialog.jsx`                                                                 | 自定义主题确认弹窗，替代浏览器原生 `window.confirm`，支持 Esc/Enter 快捷键。                                                                                                                                                                                                                                                                                                                                                                                                  |
| **HTTP 封装**      | `apiFetch.js` / `clientAuth.js` / `sharedWebSocket.js` / `wsConnection.js`          | 浏览器侧 REST 统一走 `apiFetch`；鉴权头与 WS URL 由 `clientAuth` / `buildWsUrl`；**`sharedWebSocket.js`** 负责浏览器侧 **`/ws`** 共享租约与延迟释放；WS 心跳与重连间隔常量由 `wsConnection` 提供。                                                                                                                                                                                                                                                                            |
