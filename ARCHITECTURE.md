...
| **用户壳展板**   | `UserAgentTimeline.jsx`                             | 与 **Console** 壳共用同一 `events` 流；扁平化逻辑在 **`agentTimelineModel.js`**（**`buildDisplayItems`** / **`flattenEvent`** / handlers），**`AgentTimeline.jsx`** 再导出以兼容单测。卡片式里程碑 + 动效；空态与运行中提示走 `userShell.*` i18n。                                                                                                                                                                              |
| **控制台**       | `ChatInput / Header`                                | **Agent** 模式下 **`Console` / `Agent` 界面壳**（`data-ui-shell`，`localStorage` `agentboard-ui-shell`）。**用户壳 + Agent** 不展示模式 Tab；**控制台壳**或 **Workflow/Experiment** 下展示 **`Agent` / `Workflow` / `Experiment`** 切换。主题、调色板包（含 **`claude`**）、密度、语言。Cmd/Ctrl+Enter 提交。`ChatInput` **`variant="user"`** 用于用户壳；主界面 **固定** `permissionMode: bypassPermissions`，无权限模式下拉。 |
